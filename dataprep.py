...
def spectralImages_1D(audioName, audioLoc, preprocess=False):
    audio, fs = sf.read(audioLoc)
    audioPSD  = psd(norm(audio),preprocess)
...
def spectralImages_2D(audioName, reverbloc, cleanloc,preprocess=False):
...
def prepareSimData(dataset, audiofiles, destloc, preprocess):
    ...
    for i in range(len(audiofiles)):
        ...
        if dataset == 'Train':
            ...
            cleanLoc = cleanLoc.replace('_ch1','')
        reverbPSD, cleanPSD = spectralImages_2D(audioName, audioLoc, cleanLoc, preprocess)
...
def prepareData(dataloc, destloc, dataset, preprocess=False):
    with open(dataloc+'/'+dataset+'_SimData.scp','r') as f:
        sim_audiofiles = f.readlines()
        prepareSimData(dataset, sim_audiofiles, destloc, preprocess)
    if dataset != 'Train':
        with open(dataloc+'/'+dataset+'_RealData.scp','r') as f:
            real_audiofiles = f.readlines()
            prepareRealData(dataset, real_audiofiles, destloc, preprocess)
...
if __name__=='__main__':
    ...
    parser.add_argument('--dataloc',    type=str, help='Absolute path for location of scp file (default: ./Data)', default=os.getcwd()+'/Data')
    parser.add_argument('--destloc',    type=str, help='Absolute path for location to store SpecImages (default: ./SpecImages)', default=os.getcwd()+'/SpecImages')
    parser.add_argument('--dataset',    type=str, help='List of datasets to prepare (default: \'Dev,Eval,Train\')', default='Dev,Eval,Train')
    parser.add_argument('--preprocess', type=str, help='Optimal Smoothing Preprocessing (default: False)', default=False)
    ...
    for data in dataset:
        prepareData(dataloc, destloc, data, preprocess)
