...
class SkipConvNet(pl.LightningModule): 
    ...
    def train_dataloader(self):
        TrainData   = SpecImages('/data/scratch/vkk160330/Features/Reverb_Spec/1ch/Train', mode='train')
        trainloader = DataLoader(TrainData, batch_size=4, shuffle=True,  num_workers=1)
        return trainloader

    def val_dataloader(self):
        DevData   = SpecImages('/data/scratch/vkk160330/Features/Reverb_Spec/1ch/Dev', mode='train')
        devloader = DataLoader(DevData, batch_size=4, shuffle=False,  num_workers=1)
        return devloader

    def test_dataloader(self):
        EvalData   = SpecImages('/data/scratch/vkk160330/Features/Reverb_Spec/1ch/Eval', mode='train')
        evalloader = DataLoader(EvalData, batch_size=4, shuffle=False,  num_workers=1)
        return evalloader



if __name__=='__main__':
    model = SkipConvNet().to('cuda')
